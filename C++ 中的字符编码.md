...
| 兼容性     | 兼容 ASCII                                     | 兼容 GB2312 与 ASCII，但 GBK 低字节范围（`0x40 - 0x7E`）与 ASCII 可打印字符（如 \| `0x7C`）重叠，会导致冲突，所以 **GBK 环境需规避`0x40 - 0x7E` 符号独立使用**。另外，还存在反斜杠 `\` 冲突：\ 的 ASCII 码是 `0x5C`。在 GBK 编码中，很多汉字的第二个字节恰好就是 `0x5C`（如：圳）。某些**逐字节扫描字符串的程序可能会错误地将第二个字节`0x5C`识别为一个独立的反斜杠**，从而导致路径解析错误或字符串内容被意外修改。 | 完全兼容  GBK / GB2312，支持 Unicode 映射                    |
...
`SetConsoleOutputCP(CP_UTF8);`将控制台的输出编码设置为 UTF-8。主要影响 `printf`，`std::cout` 的输出。
