...
在 Linux 和 Windows 系统中，`md5sum`、 `sha1sum`、 `sha256sum`以及 `certutil`、`Get-FileHash`是用于验证文件完整性的常用工具。

在 Linux 上，可以这样使用这些组件：
...
# 校验文件完整性：可以使用 -c 选项来校验文件的哈希值是否与之前记录的一致
# 首先，将文件的哈希值保存到一个列表文件中
sha256sum important_package.tar.gz > package_checksums.txt
# 当需要验证时（如下载文件后），使用 -c 选项
sha256sum -c package_checksums.txt

# 批量处理与脚本集成：可以使用通配符 * 来一次性计算多个文件的哈希值，并将其重定向到一个文件，方便后续批量校验
# 计算当前目录下所有 .iso 文件的 SHA256 值并保存
sha256sum *.iso > checksums.list
# 校验列表中的所有文件
sha256sum -c checksums.list
```

在 Windows 上，可以使用自带的 `certutil`工具，在 `cmd` 中就可以使用：
...
`certutil` 通常还支持更广泛的哈希算法，如 `SHA384` 和 `SHA512`。如果文件名或路径包含空格，记得用双引号将整个路径括起来，例如：`certutil -hashfile "C:\My Documents\file with spaces.zip" MD5`

在 PowerShell 中，可以使用 `Get-FileHash`这个 `cmdlet`，它能提供类似功能，并且输出对象更易于在管道命令中处理。

```powershell
Get-FileHash -Path .\large_file.iso -Algorithm SHA256
```
